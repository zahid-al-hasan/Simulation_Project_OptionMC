...
class BlackScholesAnalytical:
    ...
    def _inputs(self, t: float, spot: float | np.ndarray | None) -> tuple[np.ndarray, float]:
        if not 0 <= t <= self.T:
    ...
    def _d1(self, t: float = 0.0, spot: float | np.ndarray | None = None):
        ...
        value = (np.log(stock / self.K) + (self.r + 0.5 * self.sigma**2) * tau) / (self.sigma * np.sqrt(tau))
    ...
    def call_price(self, t: float = 0.0, spot: float | np.ndarray | None = None) -> float | np.ndarray:
        stock, tau = self._inputs(t, spot)
        if tau == 0:
            return _scalar_or_array(np.maximum(stock - self.K, 0.0))
        if self.sigma == 0:
            return _scalar_or_array(np.maximum(stock - self.K * np.exp(-self.r * tau), 0.0))
    ...
    def put_price(self, t: float = 0.0, spot: float | np.ndarray | None = None) -> float | np.ndarray:
        stock, tau = self._inputs(t, spot)
        if tau == 0:
            return _scalar_or_array(np.maximum(self.K - stock, 0.0))
        if self.sigma == 0:
            return _scalar_or_array(np.maximum(self.K * np.exp(-self.r * tau) - stock, 0.0))
        d1 = np.asarray(self._d1(t, stock))
        d2 = d1 - self.sigma * np.sqrt(tau)
